...
(async () => {
    ...
    let prefs = LC.normalize((await chrome.storage.local.get('preferences')).preferences), toastTimer;
    ...
    async function save(key, value) { const current = (await chrome.storage.local.get('preferences')).preferences; prefs = LC.normalize({ ...current, [key]: value }); await chrome.storage.local.set({ preferences: prefs }); toast('save'); }
    ...
    function settings() {
        ...
        controls.append(btn('exportStats', exportCounters), btn('clearStats', async () => { await chrome.runtime.sendMessage({ type: 'LC_CLEAR_COUNTERS' }); await refreshCounters(); toast('statsCleared'); }));
        ...
        tools.append(btn('clearCache', async () => { await chrome.runtime.sendMessage({ type: 'LC_CLEAR_CACHE' }); toast('cacheCleared'); }), btn('reset', async () => { if (confirm(t('resetConfirm'))) {
            await chrome.runtime.sendMessage({ type: 'LC_CLEAR_COUNTERS' });
    ...
    async function activeTab() { const [tab] = await chrome.tabs.query({ active: true, currentWindow: true }); return tab?.id; }
    ...
    async function exportCounters() { const data = await chrome.storage.local.get(['preferences', 'counters']); const counts = {}; for (const k of ['lists', 'previews', 'lookups', 'failures', 'cache'])
        counts[k] = Math.max(0, Math.trunc(Number(data.counters?.[k]) || 0)); download({ schemaVersion: 1, version: chrome.runtime.getManifest().version, localOnly: true, counters: counts }, 'last-comment-local-counters.json'); }
    async function refreshCounters() { const { counters = {} } = await chrome.storage.local.get('counters'); for (const n of document.querySelectorAll('[data-counter]'))
